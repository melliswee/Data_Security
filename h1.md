...
* If filename starts with '-' (or is '-'), user needs to type '<' before the filename, e.g. 'cat < -' (concatenate file -), other option is './', e.g. 'cat ./-'. Help was found from [stackoverflow](https://stackoverflow.com/questions/42187323/how-to-open-a-dashed-filename-using-terminal). **Edits from class**: if the '<' goes backwards ('>'), you'll overwrite data in the file. Dangerous!. '--' means no flags will follow. So, one can handle '-'- named file with 'cat -- -'.
